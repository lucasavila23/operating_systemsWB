...
class EventManager(threading.Thread):
    ...
    def run(self):
        ...
        while not self.clock.is_closed:
            ...
            # Next event in 10–30 sim-minutes
    ...
    def _fire(self, event: str):
        if event == "breakdown":
            ...
            log("event", "Weather", f"{cond} — slight effect on visitor mood", self.clock)
            ...
            log("event", "Security", f"⭐ VIP alert: {vip}!", self.clock)

        elif event == "flash_sale":
            restaurant = random.choice(self.park.restaurants)
            log(
                "event", "Announcements",
                f"📣 Flash deal: 30% off at '{restaurant.name}' for 10 min!",
                self.clock,
            )
            ...
            log(
                "event", "Announcements",
                "🎭 Superman stunt show in 15 min at the Main Stage — don't miss it!",
                self.clock,
            )
